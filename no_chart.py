...
def initialize_agents() -> Agent:
    ...
    web_agent = Agent(
        ...
        model=Gemini(id="gemini-2.0-flash", api_key=gemini_api_key, temperature=0.2, grounding=True),
        instructions="Focus on retrieving recent and relevant information. Summarize findings concisely in plain text. If you must emphasize, use bold text (`**text**`). Provide source URLs if available in plain text within parentheses.",  # Updated instructions
    ...
    finance_agent = Agent(
        ...
        model=Gemini(id="gemini-2.0-flash", api_key=gemini_api_key, temperature=0.1),  # Updated model
        tools=[YFinanceTools(
            stock_price=True,
        ...
        instructions=[
            ...
            "Present financial data (fundamentals, ratios, income statements) in well-formatted Markdown tables. Include the stock ticker symbol clearly at the beginning of any data related to a specific stock.",  # Clarified ticker instruction
    ...
    sentiment_agent = Agent(
        ...
        model=Gemini(id="gemini-2.0-flash", api_key=gemini_api_key, temperature=0.3),  # Updated model
    ...
    agent_team = Agent(
        ...
        model=Gemini(id="gemini-2.0-flash", api_key=gemini_api_key, temperature=0.2),  # Updated model
        instructions=[
            ...
            "5. Ensure the final response is comprehensive and directly answers the user's query, maintaining consistent formatting.",  # Added formatting instruction
...
# --- Cached Resource Initialization (No Charting Tool) ---
@st.cache_resource  # Cache the YFinanceTools instance (without stock_price for charting)
def get_limited_yfinance_tool() -> YFinanceTools:
    """Creates and caches a YFinanceTools instance with limited capabilities."""
    print("--- Initializing Limited YFinance Tool (should happen once per session) ---")  # For debugging
    return YFinanceTools() # Initialize with default False for all functionalities
...
# --- REMOVE the display_stock_chart function ENTIRELY ---
# def display_stock_chart(ticker: str, yf_tools: YFinanceTools):
#     """Fetches and displays the current stock price using markers."""
#     st.write(f"--- Generating Chart for {ticker} ---")  # Info message
#     try:
#         # Explicitly ask for the current stock price
#         price_response = agent_team.run(f"What is the current stock price of {ticker}?")
#         st.write(f"Price Response Object after Recommendation: {price_response}") # Debug
#
#         current_price = None
#         timestamp = pd.Timestamp('now', tz='Asia/Kolkata')  # Be explicit with timezone
#         st.write(f"Current Timestamp (Chennai): {timestamp}") # Debug
#
#         if hasattr(price_response, 'content'):
#             price_response_content = price_response.content.strip() # Trim leading/trailing whitespace
#             st.write(f"Price Response Content (Stripped): '{price_response_content}'") # Debug
#             if ticker.upper() in price_response_content and "current stock price" in price_response_content.lower():
#                 # Robust regex to find the numerical price
#                 price_match = re.search(r"is\s+([\d]+(?:\.[\d]+)?)", price_response_content)
#                 if price_match:
#                     current_price = float(price_match.group(1))
#                     st.write(f"Extracted Price (from price_response): {current_price}") # Debug
#
#         if current_price is not None:
#             df = pd.DataFrame({'timestamp': [timestamp], 'close': [current_price]})
#             st.write("DataFrame for Plotting:") # Debug
#             st.dataframe(df) # Debug
#
#             fig = go.Figure(data=[go.Scatter(x=df['timestamp'], y=df['close'], mode='markers')])
#             fig.update_layout(
#                 title=f'Current Stock Price for {ticker}',
#                 xaxis_title='Time',
#                 yaxis_title='Price'
#             )
...
#
#         else:
#             st.warning(f"Could not retrieve the current stock price for {ticker} to plot a chart.")
#
    ...
    for ticker in potential_tickers:
        if ticker not in common_non_tickers:
            if '---|---|' in text or 'Recommendation Trend' in text or 'Financial Data' in text:
                ...
                return ticker
            # Fallback: Return first plausible ticker even without table context
            return ticker
    return None
...
def main():
    ...
    if st.sidebar.button("ℹ️ About"):
        ...
        - **Web Agent:** Uses Google Search (grounding).
    ...
    try:
        global agent_team  # Make agent_team accessible globally
        agent_team = initialize_agents()
        # yf_tool_for_charting = get_yfinance_tool() # No longer needed
        limited_yf_tool = get_limited_yfinance_tool() # Initialize the limited YFinanceTools
    except Exception as e:
    ...
    if prompt := st.chat_input("Ask your financial question..."):
        ...
        with st.chat_message("assistant"):
            ...
            # potential_ticker_for_chart = None # No longer needed
            ...
            try:
                ...
                # --- Automatic Chart Generation (DISABLED) ---
            ...
            # --- Display chart if a ticker was identified (DISABLED) ---
